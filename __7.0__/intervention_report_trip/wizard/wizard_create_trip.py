...
class hr_analytic_timesheet_trip_wizard(osv.osv_memory):
    ...
    def create_trip(self, cr, uid, ids, context=None):
        ...
        if wiz_proxy.from_date:
            domain.append(('date_start', '>=', "%s 00:00:00" % (
                wiz_proxy.from_date,)))
                ...
                while from_parse_date != to_parse_date:  # jump lasy day
                    ...
                    for key in date_list_for_user:  # for all users:
                        ...
                        if (datetime.weekday(from_parse_date) in
                                [0, 1, 2, 3, 4]) and \
                                (day not in date_list_for_user[key]):
                            # Create empty trip
                            trip_id = trip_pool.search(cr, uid, [
                                ('date', '=', day),
                                ('user_id', '=', key),
    ...
    def on_change_month(self, cr, uid, ids, month, context=None):
        ...
        if month:
            res['value'] = {
                'from_date':
                    "%s-%02d-01 00:00:00" % (time.strftime('%Y'), int(month)),
                'to_date':
                    "%04d-%02d-01 00:00:00" % (
                        int(time.strftime('%Y')) if month != "12" else
    ...
    _defaults = {
        ...
        'month':
            ...
            if datetime.now().strftime('%m') != '01' else '12',
    }
