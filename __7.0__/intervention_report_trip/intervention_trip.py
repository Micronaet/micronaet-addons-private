...
class hr_analytic_timesheet_trip(osv.osv):
    ...
    def calculate_step_list(self, cr, uid, ids, context=None):
        ...
        # Utility functions: ##################################################
        def create_record(
                self, cr, uid, from_id, to_id, trip_id, seq, from_name,
                to_name, company_trip=False, context=None):
            """ Create record:
        ...
        for trip in self.browse(cr, uid, ids, context=context):
            ...
            # delete all
            step_ids = self.pool.get('hr.analytic.timesheet.trip.step').search(
                cr, uid, [
                    '|',
                    ('trip_id', '=', trip.id),
            ...
            for intervent in trip.intervent_ids:
                if intervent.mode == 'customer' and not \
                        intervent.extra_planned:  # only for customer
                    ...
                    if intervent.google_from == 'home':
                        ...
                        continue  # jump intervent!!! #pass
                        # TODO Error no previuos_id
                    ...
                    if intervent.google_to == 'home':
                        create_record(
                            self, cr, uid, intervent.intervent_partner_id.id,
                            home_partner_id, trip.id, seq,
                            intervent.intervent_partner_id.name, home_name,
                            context=context)
            ...
            for intervent in trip.intervent_ids:
                if intervent.mode == 'customer':  # only for customer
                    ...
                    if create_record(
                            self, cr, uid, from_id,
                            intervent.intervent_partner_id.id, trip.id,
                            seq, from_name,
                            intervent.intervent_partner_id.name, True,
                            context=context):
                        seq += 1
    ...
    def distance_between_partner(
        ...
        def distance_query(origin, destination):
            ...
            try:
                return "https://maps.googleapis.com/maps/api/distancematrix/" \
                       "json?origins=%s&destinations=%s&sensor=false" % (
                            prepare_element(
        ...
        if key not in self._api_call_cache:
            return self._api_call_cache[key]
        else:
            query = distance_query(*key)  # Generate query
            try:
                response = eval(urllib.urlopen(query).read())
            except:
                raise osv.except_osv(
                    'Google error', 'Error asking: %s' % query)
            try:
                distance_km = \
                    response['rows'][0]['elements'][0]['distance']['value'] / \
                    1000.0  # km
                self._api_call_cache[key] = distance_km
                return distance_km
            except:
                _logger.error('Error reading query: %s' % query)
                self._api_call_cache[key] = 0.0
                return 0.0
